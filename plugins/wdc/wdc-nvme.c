...
static bool get_dev_mgment_cbs_data(int fd, __u8 log_id, void **cbs_data)
{
	...
	*cbs_data = NULL;
	...
	ret = nvme_get_log14(fd, 0xFFFFFFFF, WDC_NVME_GET_DEV_MGMNT_LOG_PAGE_OPCODE,
			NVME_NO_LOG_LSP, 0, 0, false, uuid_ix, 0, WDC_C2_LOG_BUF_LEN, data);
	if (ret) {
	...
	ret = nvme_get_log14(fd, 0xFFFFFFFF, WDC_NVME_GET_DEV_MGMNT_LOG_PAGE_OPCODE,
			NVME_NO_LOG_LSP, 0, 0, false, uuid_ix, 0, le32_to_cpu(hdr_ptr->length), data);
		...
		if ((*cust_id == WDC_CUSTOMER_ID_0x1004) || (*cust_id == WDC_CUSTOMER_ID_0x1005))
		{
			if (uuid_index == 0)
			{
				...
				ret = nvme_get_log14(fd, 0xFFFFFFFF, WDC_NVME_GET_EOL_STATUS_LOG_OPCODE,
						NVME_NO_LOG_LSP, 0, 0, false, uuid_index, 0, WDC_NVME_SMART_CLOUD_ATTR_LEN, data);
				...
				/* Get the 0xC0 log data */
				ret = nvme_get_log14(fd, 0xFFFFFFFF, WDC_NVME_GET_EOL_STATUS_LOG_OPCODE,
						NVME_NO_LOG_LSP, 0, 0, false, uuid_index, 0, WDC_NVME_EOL_STATUS_LOG_LEN, data);
